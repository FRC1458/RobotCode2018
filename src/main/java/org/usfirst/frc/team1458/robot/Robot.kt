    ...
    override fun setupAutoModes() {
        addAutoMode(AutoMode.create {
            ...
            while(Math.floor((systemTimeMillis-startTime)/50.0)<leftTrajectory.length()){
                ...
                delay(1)
                System.out.println("$index,${leftTrajectory[index].velocity/10.0},${righttTrajectory[index].velocity/10.0},${leftTrajectory.length()},${drivetrain.leftMaster._talonInstance?.getClosedLoopError(0)},${drivetrain.rightMaster._talonInstance?.getClosedLoopError(0)},${drivetrain.leftMaster.connectedEncoder.rate},${drivetrain.rightMaster.connectedEncoder.rate}")
            }
